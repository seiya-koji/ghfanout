...
uv run coverage run -m pytest               # Tests with coverage measurement
uv run coverage report                      # Coverage summary (fails below 100%)
...
CI runs the tests across Python 3.11–3.14, plus ruff, mypy, and a strict docs build on every pull request — all of them should pass locally before you open one. Coverage is measured on every Python version, combined into a single report that must stay at 100%, and uploaded to [Codecov](https://codecov.io/gh/seiya-koji/ghfanout).
